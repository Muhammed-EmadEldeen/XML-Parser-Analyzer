...
unordered_map<string, int> count_byte_pairs(const string& input) {
    ...
    for (size_t i = 0; i < input.length() - 1; ++i) {
        ...
        // check if the pair is already in the mapping "very important" to avoid infinite loop
...
string replace_byte_pair(string& input, const string& pair, const string& placeholder) {
...
string byte_pair_compress(const string& input) {
    ...
    while (!placeholders.empty()) {
        ...
        if (max_frequency < 2) break; // exit if all pairs are unique
